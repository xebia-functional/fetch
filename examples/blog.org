...
the combinators that ~Fetch~ offers. The most simple one is ~sequence~, given a list of fetches
of the same type it will run them concurrently, batching whatever it can:

#+BEGIN_SRC scala
val fetch: Fetch[List[Article]] = Fetch.sequence(List(getArticle(1), getArticle(2)))
...
A similar combinator is ~traverse~, which is the combination of ~map~ and ~sequence~:
